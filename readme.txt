...
### 问题 1：index_db.py 与哪个实验相关？

  根据实验指导 PDF 附件 1 的评分标准，索引属于选做加分项，不归入三个主实验（实验1 存储、实验2 SQL 解析、实验3
  日志）。原文：

> **"（3）索引的实现（如B树、哈希等，要求有索引和无索引的对比查询效果）"**

  index_db.py 目前是 B 树骨架，insert_index_entry()仅实现了单层叶子节点的插入，**分裂（split）、内部节点维护、范围查询**均未实现。
